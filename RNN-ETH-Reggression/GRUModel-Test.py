...
def main():
    ...
    model_gru = GRUModel(input_dim=input_dim,
    ...
    train_loss, valid_loss = fit_train(n_epochs=n_epochs, train_dl=train_dl, valid_dl=valid_dl,
                                       model=model_gru, loss_function=loss_function, optimizer=optimizer,
                                       batch_size=batch_size, input_dim=input_dim)

    # Figure for loss
    make_plot(train_loss=train_loss, valid_loss=valid_loss, model_name='GRU')

    predictions, values = test_fit(model = model_gru, test_dl_one= test_dl_one, input_dim=input_dim)  # Test the model

    df_result_gru = format_predictions(predictions, values, ETH[1558:], scaler)
    print(df_result_gru)
    ...
    plot_predictions(df_result_gru, model_name = "gru")

    # Difine Days, Evaluate , make df result
    future_days = 240

    preds = make_future_preds(x_test=x_test, model=model_gru, future_days=future_days)
    future_df_gru = make_future_df(preds, df_result_gru, future_days,scaler = scaler)
    ...
    plot_predictions(df_result_gru, future_pred=future_df_gru, model_name="gru")
